...
def dump_vehicle_state(vehicle) -> None:
    ...
    log("----- GPS -----")
    log(f"Fix Type: {vehicle.gps_0.fix_type}")
    ...
    log("----- BATTERY -----")
    log(f"Voltage: {vehicle.battery.voltage}")
    log(f"Level: {vehicle.battery.level}")

    log("----- PARAMETERS -----")
    for p in ["ARMING_CHECK", "ARMING_REQUIRE", "BRD_SAFETYENABLE"]:
        try:
            log(f"{p}: {vehicle.parameters[p]}")
        except Exception:
            log(f"{p}: NOT FOUND")


def attempt_arm(vehicle) -> None:
    log("===== ARM TEST =====")

    vehicle.mode = VehicleMode("GUIDED")
    time.sleep(2)
    log(f"Mode after set: {vehicle.mode.name}")

    vehicle.armed = True
    time.sleep(3)
    log(f"Armed status: {vehicle.armed}")
